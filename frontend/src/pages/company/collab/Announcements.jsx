...
import { Plus, Search, Pencil, Trash2, Download, RefreshCw, UserX, Megaphone } from 'lucide-react'
...
const Announcements = () => {
    ...
    const filteredAnnouncements = announcements.filter(announcement => {
        ...
        const matchesStatus = showInactive ? announcement.status === 'Inactivo' : true
    ...
    const toggleStatus = (id) => {
        setAnnouncements(announcements.map(a =>
            a.id === id
            ? { ...a, status: a.status === 'Activo' ? 'Inactivo' : 'Activo' } : a 
        ))
        toast.success('Estado actualizado')
    }

    const handleExport = () => {
        toast.success('Exportando anuncios...')
    }
            ...
            <div className="w-full max-w-6xl px-4">
                                ...
                                {filteredAnnouncements.map((announcement, index) => (
                                    <tr key={announcement.id} className={`transition-all hover:bg-gray-50 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                                        ...
                                        <td className="px-6 py-4">
                                            ...
                                            >
                                                <span className={`w-2 h-2 ${announcement.status === 'Activo' ? 'bg-emerald-500' : 'bg-red-500'} rounded-full mr-2`}></span>
                                                {announcement.status}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center justify-end space-x-2">
                                                <button
                                                    ...
                                                    title={announcement.status === 'Activo' ? 'Desactivar' : 'Activar'}
                                                >
                                                    <UserX className="w-4 h-4 text-orange-600" />
                                                </button>
                                                <button 
                                                    onClick={() => toast.success('Editando anuncio...')}
                ...
                <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="bg-gradient-to-br from-emerald-500 to-green-600 rounded-xl p-5 text-center shadow-lg">
                        ...
                        <p className="text-2xl font-bold text-white">{announcements.filter(a => a.status === 'Activo').length}</p>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-red-600 to-red-700 rounded-xl p-5 text-center shadow-lg">
                        ...
                        <p className="text-2xl font-bold text-white">{announcements.filter(a => a.status === 'Inactivo').length}</p>
                    </div>
                </div>
