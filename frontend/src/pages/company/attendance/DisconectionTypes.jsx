...
import { Plus, Search, Pencil, Trash2, Download, RefreshCw, UserX, Wifi, WifiOff } from 'lucide-react'
import toast from 'react-hot-toast';

const DisconectionTypes = () => {
    ...
    const filteredTypes = types.filter(type => {
        ...
        const matchesStatus = showInactive ? type.status === 'Inactivo' : true
    ...
    const toggleStatus = (id) => {
        setTypes(types.map(t =>
            t.id === id
            ? { ...t, status: t.status === 'Activo' ? 'Inactivo' : 'Activo' } : t 
        ))
        toast.success('Estado actualizado')
    }

    const handleExport = () => {
        toast.success('Exportando tipos de desconexión...')
    }
            ...
            <div className="w-full max-w-6xl px-4">
                ...
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-600">
                            Configura los tipos de desconexión disponibles
                        </p>
                    </div>
                </div>
                ...
                <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-5 mb-6">
                    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center space-y-3 lg:space-y-0 gap-4">
                        ...
                        <div className="relative flex-1 w-full lg:max-w-md">
                            ...
                            <input
                                type="text"
                                placeholder="Buscar tipos de desconexión..."
                                value={searchTerm}
                                ...
                                {filteredTypes.map((type, index) => (
                                    <tr key={type.id} className={`transition-all hover:bg-gray-50 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                                        ...
                                        <td className="px-6 py-4">
                                            ...
                                            >
                                                <span className={`w-2 h-2 ${type.status === 'Activo' ? 'bg-emerald-500' : 'bg-red-500'} rounded-full mr-2`}></span>
                                                {type.status}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center justify-end space-x-2">
                                                <button
                                                    ...
                                                    title={type.status === 'Activo' ? 'Desactivar' : 'Activar'}
                                                >
                                                    <UserX className="w-4 h-4 text-orange-600" />
                                                </button>
                                                <button 
                                                    onClick={() => toast.success('Editando tipo...')}
                ...
                <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl p-5 text-center shadow-lg">
                        <Wifi className="w-6 h-6 text-white mx-auto mb-2" />
                    ...
                    <div className="bg-gradient-to-br from-emerald-500 to-green-600 rounded-xl p-5 text-center shadow-lg">
                        ...
                        <p className="text-2xl font-bold text-white">{types.filter(t => t.status === 'Activo').length}</p>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-red-600 to-red-700 rounded-xl p-5 text-center shadow-lg">
                        ...
                        <p className="text-2xl font-bold text-white">{types.filter(t => t.status === 'Inactivo').length}</p>
                    </div>
                </div>
...
export default DisconectionTypes;